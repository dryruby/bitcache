module Bitcache::ZeroMQ
  ...
  class Client
    ...
    ##
    # @return [Array<String>]
    def read_reps(nonblock = false)
      if rep = read_rep(nonblock)
        reps = [rep]
        reps << read_rep(nonblock) while @req.more_parts?
        reps
      end
    end
    ...
    ##
    # @return [String]
    def read_rep(nonblock = false)
      @req.recv_string(nonblock ? ZMQ::NOBLOCK : 0)
    end
