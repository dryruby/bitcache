module Bitcache
  ...
  class Identifier < Struct
    ...
    def initialize(digest = nil)
      ...
      @digest.force_encoding(Encoding::BINARY) if @digest.respond_to?(:force_encoding) && @digest.encoding != Encoding::BINARY # for Ruby 1.9+
    end
    ...
    def to_s(base = 16)
      case base
        ...
        when 2  then digest.unpack('B*').first
        else raise ArgumentError, "invalid radix #{base}"
      end
    end
